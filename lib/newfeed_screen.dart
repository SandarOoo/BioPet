...
if (_isPosting || _isCheckingImages) {
return;
}
...
final ClassificationProvider classifier =
...
try {
prepared = await _preparePickedImage(
pickedImages[index],
index,
);

final result =
await classifier.validatePetImageForPost(
prepared.path,
);
...
'petProbability=${result.petProbability}, '
...
await prepared.delete().catchError(
(_) => prepared!,
);

rejectedMessages.add(result.message);
}
} catch (e) {
if (prepared != null &&
...
: 'ခွေး သို့မဟုတ် ကြောင်ပုံကိုသာ ရွေးနိုင်ပါသည်။';
} else if (rejectedMessages.isNotEmpty) {
_error =
'ခွေး/ကြောင်မဟုတ်သော သို့မဟုတ် မသေချာသောပုံ '
'${rejectedMessages.length} ပုံကို မထည့်ထားပါ။';
} else {
_error = null;
}
});
...
if (image.path.contains('.biopet_')) {
await image.delete().catchError(
(_) => image,
);
}
...
debugPrint('LOCAL IMAGE CHECK ERROR: $e');
...
setState(() {
_error =
'ဖုန်းထဲက AI model ဖြင့် ပုံစစ်၍မရပါ။ '
'App ကိုပိတ်ပြီး ပြန်ဖွင့်ကာ ထပ်စမ်းပါ။\n$e';
});
} finally {
if (mounted) {
setState(() {
...
'${responseCode ?? ''} ${responseMessage ?? ''} $raw'
.toLowerCase();

if (searchable.contains('openrouter') ||
searchable.contains('openai') ||
searchable.contains('gemini')) {
return 'Railway မှာ image moderation controller အဟောင်းကို '
'အသုံးပြုနေသေးပါသည်။ ပြင်ထားသော postController.js ကို '
'push လုပ်ပြီး deployment ပြီးအောင် စောင့်ပါ။';
}

if (searchable.contains('application failed to respond') ||
searchable.contains('502')) {
return 'Railway backend က response မပြန်နိုင်သေးပါ။ '
'Deployment ပြီးမှ ပြန်တင်ပါ။';
}

final String message =
(responseMessage ?? raw).trim();
